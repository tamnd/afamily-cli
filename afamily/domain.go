...
func (Domain) Info() kit.DomainInfo {
	return kit.DomainInfo{
		...
		Identity: kit.Identity{
			Binary: "afamily",
			Short:  "Read public Afamily (afamily.vn) lifestyle articles.",
			Long: `Read public Afamily (afamily.vn) lifestyle articles.

afamily reads from Afamily RSS feeds — no API key, no browser required.
...
// Classify turns an Afamily URL or numeric ID into (type, id).
func (Domain) Classify(input string) (uriType, id string, err error) {
	input = strings.TrimSpace(input)
	if input == "" {
		return "", "", errs.Usage("empty Afamily reference")
	}
	if strings.Contains(input, "afamily.vn/") {
