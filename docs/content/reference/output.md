...
describes what users get. Pick a format with `-o`, or let afamily choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
afamily <command> -o table   # aligned columns for reading
afamily <command> -o jsonl   # one JSON object per line, for piping
afamily <command> -o json    # a single JSON array
afamily <command> -o csv     # spreadsheet friendly
afamily <command> -o tsv     # tab-separated
afamily <command> -o url     # just the URL column
afamily <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
afamily <command> --fields id,title,url
```
...
```bash
afamily <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
afamily <command>            # a table, because this is a terminal
afamily <command> | wc -l    # JSONL, because this is a pipe
```
